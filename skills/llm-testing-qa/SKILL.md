...
- `test_03_security_hallucination.py` — DeepEval 五指标全覆盖（Faithfulness / AnswerRelevancy / Hallucination / Toxicity / GEval）；含 4.x 统一语义防坑 + Hallucination vs Faithfulness 区分 + GEval evaluation_steps 写法
- `test_04_agent_trajectory.py` — Agent 轨迹评测：轨迹断言（工具名/参数/顺序/终态/无死循环）+ stochastic run×N 通过率门禁 + pass^k 可靠性（τ-bench）+ LLM-Judge 慢测（GEval evaluation_steps）+ 多 Agent 协作链路评测（AgentBench POMDP 任务结构）
...
- **DeepEval 4.x Hallucination/Toxicity 不是"越低越好"**——4.x 已统一为"非幻觉比例/非毒性比例"，score 全部越高越好、threshold 是最低通过线。第三方旧教程说"越低越好"是 v0/v1 过时写法。
...
- GEval 用 `criteria` 跨次运行有 ±0.05 浮动；回归测试推荐用 `evaluation_steps` 列表，更稳定。
- AgentBench / τ-bench 是"参照设计模式"
